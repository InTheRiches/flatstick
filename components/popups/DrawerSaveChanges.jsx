...
const DrawerSaveChanges = ({saveChangesRef, show, disabled}) => {
    ...
    useEffect(() => {
        if (show)
            saveChangesRef.current.expand()
        else
            saveChangesRef.current.close()
    }, [show]);

    // renders
    return (
        <BottomSheet index={-1} ref={saveChangesRef}
            ...
            <BottomSheetView style={{paddingBottom: 12, backgroundColor: colors.background.secondary}}>
                <View style={{
                    ...
                    <Text style={{fontSize: 20, fontWeight: 500, color: colors.text.primary}}>Save
                        your changes?</Text>
                    <View style={{flexDirection: "row", justifyContent: "space-between", marginTop: 16}}>
                        ...
                        <PrimaryButton disabled={disabled} title={"Save"}
                                       onPress={() => {
