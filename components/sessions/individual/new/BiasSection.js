    ...
    return (
        <>
            ...
            <View style={{ marginTop: 20 }}>
                <LeftRightBias bias={(session.meta.type === "full" || session.meta.type === "green") && session.meta.units === 0 ? roundTo(session.stats.leftRightBias/12, 1) : session.stats.leftRightBias} units={session.meta.units} />
                <ShortPastBias bias={(session.meta.type === "full" || session.meta.type === "green") && session.meta.units === 0 ? roundTo(session.stats.shortPastBias/12, 1) : session.stats.shortPastBias} units={session.meta.units} />
            </View>
        </>
    );
